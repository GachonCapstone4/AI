...
from src.metrics import record_classify_error, record_classify_success
...
def _model_version_from_runtime(runtime, bundle: dict | None = None) -> str:
    if hasattr(runtime, "current_model_version"):
        version = getattr(runtime, "current_model_version", None)
        if version:
            return version
    ...
    if bundle is not None:
        ...
        version = (
            bundle_runtime.get("active_model_version")
            or bundle_runtime.get("metadata_model_version")
        )
        if version:
            return version
    ...
    if isinstance(runtime, dict):
        ...
        bundle_runtime = model.get("runtime") or {}
        return (
            bundle_runtime.get("active_model_version")
            or bundle_runtime.get("metadata_model_version")
            or "unknown"
...
def run_classify(payload: ClassifyRequest, runtime) -> ClassifyResponse:
    ...
    bundle = None
    model_version = _model_version_from_runtime(runtime)
    ...
    try:
        ...
        model_version = _model_version_from_runtime(runtime, bundle)
        ...
        if raw_schedule is not None:
            ...
            date, parsed_time = parse_datetime_kst(date_text, time_text, payload.received_at)
            schedule_info = {
                "date": date,
                "time": parsed_time,
        ...
        record_classify_success(
            ...
            domain=response.classification.domain,
            intent=response.classification.intent,
            latency_seconds=time.perf_counter() - start,
            confidence_score=response.confidence_score,
            schedule_detected=bool(
                schedule_info and (schedule_info.get("date") or schedule_info.get("time"))
            ),
        )
        return response
