...
이메일의 <b>업무 분야(Domain)</b>와 <b>세부 요청 의도(Intent)</b>를 자동 분류하고,  
...
| **애매한 Domain** | "저번 행사 영수증 처리 됐나요?" → Admin(행사운영)? Finance(비용처리)? |
| **애매한 Intent** | "비밀번호 까먹었는데" → IT(계정관리)? Admin(총무)? |
...
| **애매한 Intent** | 도메인 경계가 모호한 입력에서 confidence가 어떻게 나타나는가 |
| **일정 추출** | 자연어 일정 표현의 edge case를 올바르게 파싱하는가 |
...
> 실제 업무용 이메일 데이터셋을 구하기 어려워,  
> **LLM을 활용해 카테고리별 프롬프트 기반으로  
> 학습 데이터를 직접 생성**했습니다.
